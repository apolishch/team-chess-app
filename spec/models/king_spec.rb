...
RSpec.describe King, type: :model do
  describe "king basic movement" do
    it "should move either horizontally or vertically or move diagonally but only one square" do
        ...
        expect(dummy_king.right_move?(6,2)).to eql(false)
        expect(dummy_king.right_move?(5,0)).to eql(true)
        expect(dummy_king.right_move?(4,1)).to eql(true)
        expect(dummy_king.right_move?(3,1)).to eql(true)
        expect(dummy_king.right_move?(0,1)).to eql(false)
    end
